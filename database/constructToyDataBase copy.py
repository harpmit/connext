...
for userID,name,profileImageSrc in zip(range(len(firstNames)),firstNames,profileImageSrcs):
    profiles.append(Profile(userID+1,name,randomDraw(lastNames),randomDraw(industries),randomDraw(schools),profileImageSrc))
...
similarityScores = pd.DataFrame(similarityScores,
                                index=[str(i) for i in np.arange(1,len(similarityScores)+1)],
                                columns=np.arange(1,len(similarityScores)+1))

for counter,profile in enumerate(profiles):
    similarities = similarityScores[counter+1].to_dict()
